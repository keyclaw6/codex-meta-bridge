 ...
 * as a separate Linux process, then drives it over real HTTP — OAuth flow,
...
import { findPidsOnPort } from "../src/proc.mjs";
...
try {
  ...
  await client.connect(new StreamableHTTPClientTransport(new URL(`${base}/mcp/${TOKEN}`)));
  {
    ...
    check("exact 14-tool schema is live", JSON.stringify(tools) === JSON.stringify(expectedTools), tools.join(","));
    ...
    check("status exposes observability fields", Object.hasOwn(s, "active_command") && Array.isArray(s.subagent_threads));
  }
